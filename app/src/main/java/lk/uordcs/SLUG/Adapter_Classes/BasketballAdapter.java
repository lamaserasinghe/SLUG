...
public class BasketballAdapter extends RecyclerView.Adapter<BasketballAdapter.BasketballViewHolder> implements Filterable {
    private Context mCtx;

    private List<BasketballResults> resultsListFull;
    private List<BasketballResults> resultsList;

    public BasketballAdapter(Context mCtx, List<BasketballResults> resultsList) {
        this.mCtx = mCtx;
        this.resultsList = resultsList;
        resultsListFull=new ArrayList<>(resultsList);
    }

    public BasketballViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //inflating and returning our view holder
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        View view = inflater.inflate(R.layout.basketball_result_card, null);
        return new BasketballViewHolder(view);
    }

    public void onBindViewHolder(BasketballViewHolder holder, int position) {
        //getting the product of the specified position
        BasketballResults results = resultsList.get(position);
    ...
    class BasketballViewHolder extends RecyclerView.ViewHolder {
        ...
        public BasketballViewHolder(View itemView) {
            super(itemView);
    ...
    private Filter resultFilter=new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<BasketballResults> filteredList=new ArrayList<>();
            ...
            if(constraint==null || constraint.length()==0){
                ...
                for(BasketballResults item:resultsListFull){
